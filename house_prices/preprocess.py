...
def encoding_with_one_hot_encoder(data, feature_1, feature_2):
    fitting_encoder = ohe.fit(data[[feature_1,
                                    feature_2]]).transform(data[[feature_1,
                                                                 feature_2]])
    col_names = ohe.get_feature_names_out(input_features=[feature_1,
                                                          feature_2])
...
def scaling_with_standard_scalar(data, feature_1, feature_2):
    data[[feature_1,
          feature_2]] = sc.fit(data[[feature_1,
                                     feature_2]]).transform(data[[feature_1,
                                                                  feature_2]])
    return data
...
def encoder_with_transform(data, feature_1, feature_2):
...
def scaling_with_transform(data, feature_1, feature_2):
...
def compute_rmsle(y_val: np.ndarray,
                  y_pred: np.ndarray,
